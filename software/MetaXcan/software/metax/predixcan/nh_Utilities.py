...
try:
    from .. expression import nh_HDF5Expression as HDF5Expression 
except:
...
class PContext(_PContext):
    def __init__(self, args, sidx, expression):
        self.args = args
        self.mode = None
        self.covariates = None
        self.pheno = None
...
def expression_from_args(args, sidx, prediction_results = None):
    if prediction_results:
        ...
        expression = HDF5Expression.Expression(args.hdf5_expression_file, sidx)
...
def p_context_from_args(args, sidx, prediction_results=None):
    _check_args_file(args)
    expression = expression_from_args(args, sidx, prediction_results)
    context = PContext(args, sidx, expression)
    return context
...
def _prepare_phenotype(context):
    ...
    context.pheno = _pheno_from_file_and_column(context.args.input_phenos_file, context.args.input_phenos_column, context.sidx, context.args.input_phenos_na_values)
    ...
    if context.args.covariates_file and context.args.covariates:
        ...
        context.covariates = _get_covariates(context.args, context.sidx)
        logging.info("Replacing phenotype with residuals")
        context.pheno = _get_residual(context.pheno, context.covariates)

def _pheno_from_file_and_column(path, column, sidx, na_rep=None):
...
def _get_covariates(args, sidx):
    ...
    covariates = covariates.loc[covariates.index.isin(sidx)]
    covariates = covariates.loc[covariates.index.isin(sidx)]
